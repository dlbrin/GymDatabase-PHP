...
/*DATABASE FOLDER and FILE NAME TO BACKUP*/
define("BACKUP_DIR", './backupDB/');
define("BACKUP_FILE",BACKUP_DIR."back_".DB_NAME."-".date("l jS \of F Y h:i:s A").".sql.gz");
...
class Backup_Database {
    ...
    public function __construct($host, $username, $passwd, $dbName, $charset = 'utf8') {
        ...
        //$this->backupFile              = 'back_'.$this->dbName.'-'.date("Ymd_His", time()).'.sql';
        $this->backupFile              = 'back_'.$this->dbName.'-'.date("Ymd").'.sql';
...
/*
Check the backup folder path, if today backup is created or not
*/
$path=BACKUP_FILE;
if (FILE_EXISTS($path))
die("");
